...
const PaymentProofs = () => {
  ...
  const [showPaymentProofId, setShowPaymentProofId] = useState(null); // State to track which row to show the component for
  ...
  useEffect(() => {
    if (singlePaymentProof && Object.keys(singlePaymentProof).length > 0) {
      ...
      setShowPaymentProofId(singlePaymentProof._id);
    }
  }, [singlePaymentProof]);
              ...
              paymentProofs.map((element, index) => {
                return (
                  <>
                    ...
                    {/* Show the UpdatePayment component only below the clicked row */}
                    {showPaymentProofId === element._id && (
                      <tr>
                        <td colSpan="3">
                          <UpdatePayment
                            setShowPaymentProof={() =>
                              setShowPaymentProofId(null)
                            }
